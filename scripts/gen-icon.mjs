...
/**
 * Generates assets/icon.png — Poly (the purple block alien) on a dark rounded
 * square — from the same grid as the TUI/GUI mascot. Dependency-free PNG encoder
 * (zlib only). Run with: node scripts/gen-icon.mjs
 */
const POLY_ART = [
  ".##...........##.",
  "...##.......##...",
...
for (let y = 0; y < N; y++) {
  for (let x = 0; x < N; x++) {
    set(x, y, inRounded(x, y) ? BG : [0, 0, 0, 0]);
  }
}
  ...
  for (let gx = 0; gx < COLS; gx++) {
    ...
    for (let dy = 0; dy < cell; dy++) {
      for (let dx = 0; dx < cell; dx++) {
        set(ox + gx * cell + dx, oy + gy * cell + dy, PURPLE);
      }
    }
  }
}
